...
def predict():
    ...
    language = request.args.get('lang', 'en')  # Get language from query parameter
    ...
    telugu_name = knowledge_base.TELUGU_NAMES.get(name, name)
    info = knowledge_base.pest_info.get(name, {}).get(language, knowledge_base.pest_info.get(name, {}).get('en', {}))

    print(f"DEBUG - Detected pest name: {name}")
    print(f"DEBUG - Telugu name from TELUGU_NAMES: {telugu_name}")
    print(f"DEBUG - All available names: {knowledge_base.TELUGU_NAMES}")
